...
class BaseDataProcessor:
    ...
    def _get_embedding(self, description):
        if self.language_embedding == 'binary_encoder':
            return lang.binary_encoder(description, embedding_dim=self.language_embedding_dim)
        elif self.language_embedding == 'universal_sentence_encoder':
            return lang.universe_sentence_encoder(
                description, embedding_dim=self.language_embedding_dim
            )
        else:
            raise Exception("Unsupported language embedding")

    def _write_file(self, target_file: str, episodic_data: EpisodicDataClass) -> None:
        ...
        with h5py.File(target_file, 'w') as root:
            ...
            root.create_dataset('sentence_embedding', sentence_embedding.shape, dtype=np.float32)
            ...
            for key, value in qpos.items():
                qpos_group.create_dataset(key, value.shape, dtype=value.dtype)
                qpos_group[key][...] = value

            for key, value in qvel.items():
                qvel_group.create_dataset(key, value.shape, dtype=value.dtype)
                qvel_group[key][...] = value

            for key, value in images.items():
                images_group.create_dataset(key, value.shape, dtype=value.dtype)
                images_group[key][...] = value

            for key, value in actions.items():
                actions_group.create_dataset(key, value.shape, dtype=value.dtype)
