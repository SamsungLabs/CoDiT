...
class SrrcDualFrankasDataProcessor(BaseDataProcessor):
    ...
    def _generate_episodic_data(self):
        ...
        for idx, source_folder in tqdm.tqdm(enumerate(source_folders)):
            ...
            target_file = source_folder.replace(
                self.source_dir, os.path.join(self.target_dir, os.path.basename(self.source_dir))
            )
    ...
    def _read_file(self, npy_files) -> EpisodicDataClass:
        ...
        for npy_file in npy_files:
            ...
            qpos_dict['left_arm_qpos'] = np.append(
                qpos_dict['left_arm_qpos'], left_arm_qpos, axis=0
            )
            left_gripper_qpos = np.expand_dims(root['obs']['joint_position/left_hand'], axis=0)
            qpos_dict['left_gripper_qpos'] = np.append(
                qpos_dict['left_gripper_qpos'], left_gripper_qpos, axis=0
            )
            right_arm_qpos = np.expand_dims(root['obs']['joint_position/right_arm'], axis=0)
            qpos_dict['right_arm_qpos'] = np.append(
                qpos_dict['right_arm_qpos'], right_arm_qpos, axis=0
            )
            right_gripper_qpos = np.expand_dims(root['obs']['joint_position/right_hand'], axis=0)
            qpos_dict['right_gripper_qpos'] = np.append(
                qpos_dict['right_gripper_qpos'], right_gripper_qpos, axis=0
            )

            left_arm_qvel = np.expand_dims(root['obs']['joint_velocity/left_arm'], axis=0)
            qvel_dict['left_arm_qvel'] = np.append(
                qvel_dict['left_arm_qvel'], left_arm_qvel, axis=0
            )
            left_gripper_qvel = np.expand_dims(root['obs']['joint_velocity/left_hand'], axis=0)
            qvel_dict['left_gripper_qvel'] = np.append(
                qvel_dict['left_gripper_qvel'], left_gripper_qvel, axis=0
            )
            right_arm_qvel = np.expand_dims(root['obs']['joint_velocity/right_arm'], axis=0)
            qvel_dict['right_arm_qvel'] = np.append(
                qvel_dict['right_arm_qvel'], right_arm_qvel, axis=0
            )
            right_gripper_qvel = np.expand_dims(root['obs']['joint_velocity/right_hand'], axis=0)
            qvel_dict['right_gripper_qvel'] = np.append(
                qvel_dict['right_gripper_qvel'], right_gripper_qvel, axis=0
            )
            ...
            act_dict['left_arm_qpos'] = np.append(
                act_dict['left_arm_qpos'], action_left_arm_qpos, axis=0
            )
            action_left_gripper_qpos = np.expand_dims(
                root['action']['joint_command/left_hand'], axis=0
            )
            act_dict['left_gripper_qpos'] = np.append(
                act_dict['left_gripper_qpos'], action_left_gripper_qpos, axis=0
            )
            action_right_arm_qpos = np.expand_dims(
                root['action']['joint_command/right_arm'], axis=0
            )
            act_dict['right_arm_qpos'] = np.append(
                act_dict['right_arm_qpos'], action_right_arm_qpos, axis=0
            )
            action_right_gripper_qpos = np.expand_dims(
                root['action']['joint_command/right_hand'], axis=0
            )
            act_dict['right_gripper_qpos'] = np.append(
                act_dict['right_gripper_qpos'], action_right_gripper_qpos, axis=0
            )
