    ...
    def create_observations_from_dict(self, obs_dict: Dict):
        ...
        for key, value in self.observation_space.items():
            ...
            if not obs_dict[key].dtype == value.dtype:
                raise Exception(
                    f"Observation dtype mismatch between space ({value.dtype}) and input_dict ({obs_dict[key].dtype})"
                )
            if not obs_dict[key].shape[-len(value.shape) :] == value.shape:
                raise Exception(
                    f"Observation shape mismatch between space ({value.shape}) and input_dict ({obs_dict[key].shape})"
                )
    ...
    def create_actions_from_dict(self, act_dict: Dict):
        ...
        for key, value in self.action_space.items():
            ...
            if not act_dict[key].dtype == value.dtype:
                raise Exception(
                    f"Action dtype mismatch between space ({value.dtype}) and input_dict ({act_dict[key].dtype})"
                )
            if not act_dict[key].shape[-len(value.shape) :] == value.shape:
                raise Exception(
                    f"Action shape mismatch between space ({value.shape}) and input_dict ({act_dict[key].shape})"
                )
